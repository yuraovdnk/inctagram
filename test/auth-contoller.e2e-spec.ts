...
describe('AuthController (e2e)', () => {
  ...
  describe('POST:[HOST]/auth/password-recovery', () => {
    it('should return code 400 If email is incorrect', async () => {
      await request(app.getHttpServer())
    ...
    it('should return code 204 If the email is correct', async () => {
      await request(app.getHttpServer())
        .post('/auth/password-recovery')
        .send({
          email: users[0].email,
    ...
    it('should return code 204 If the email is correct but email is not in dataBase', async () => {
      await request(app.getHttpServer())
        .post('/auth/password-recovery')
        .send({
    ...
    it('should return code 429 If More than 5 attempts from one IP-address during 10 seconds', async () => {
      await request(app.getHttpServer())
        .post('/auth/password-recovery')
        .send({
          email: 'email1111@gmail.com',
  ...
  describe('POST:[HOST]/auth/new-password', () => {
    it('should return code 400 If the inputModel is incorrect', async () => {
      await request(app.getHttpServer())
    ...
    it('should return code 400 If the inputModel has incorrect value (for incorrect password length) ', async () => {
    ...
    it('should return code 400 If  RecoveryCode is incorrect', async () => {
      await request(app.getHttpServer())
        .post('/auth/new-password')
        .send({
          newPassword: 'string',
    ...
    it('should return code 204 If code is valid and new password is accepted', async () => {
      const recoveryCode = await dbTestHelper.getPasswordRecoveryCode(
        users[0].id,
      );
      const res = await request(app.getHttpServer())
